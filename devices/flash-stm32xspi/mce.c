...
/* Digest 4 bytes of data according to CRC-8-CCITT algorithm.
 * The data is digested in big-endian manner (MSByte first). */
static u8 crc8_consumeWord(u32 data)
{
	const u32 poly = 0x07 << 24;
	size_t i;
	u32 crc = data;
	for (i = 0; i < 32; i++) {
 ...
 * Returns obfuscated CRC-8 as produced by the MCE */
static u8 mce_storeKey(mce_t per, u32 reg, const u8 *key, u32 keysize)
{
	static const u8 obfuscation[4] = { 0xaaU, 0x03U, 0x18U, 0xc0U };
	size_t i;
	u32 regVal;
	u8 crc = 0x55U;
	...
	for (i = 0; i < keysize; i += 4) {
		regVal =
				...
				((u32)key[i + 2] << 16) |
				((u32)key[i + 3] << 24);
		*(mce_setup[per].base + (reg + (i / 4))) = regVal;
		/* XOR the register value with obfuscations introduced by ST */
		regVal ^=
				((u32)crc) |
				((u32)obfuscation[i / 4] << 8) |
				((u32)crc << 16) |
				((u32)obfuscation[i / 4] << 24);
		crc = crc8_consumeWord(regVal);
	}
...
static int mce_configureCipherContextInternal(mce_t per, u8 ctxid, u16 version, u32 mode, const u8 *nonce, const u8 *ctxkey)
{
	...
	/* If context already in use, return error */
	...
	*cccfgr = t;
	...
	if (ctxkey != NULL) {
		...
		crc = mce_storeKey(per, cckeyr0, ctxkey, 16);
		hal_cpuDataMemoryBarrier();
		/* Compare CRC computed by CPU to the CRC computed by MCE */
		if (((*(mce_setup[per].base + MCE_CCCFGR(ctxid)) >> 8) & 0xffU) != crc) {
			return -EIO;
		}
...
int mce_configureRegion(int dev, mce_reg_t reg, const dev_memcrypt_args_t *args)
{
	...
	u32 ctxid = 1; /* Always set to 1 since we only allow a single region in the controller anyway */
	u32 cipher;
	mce_t per;
