...
static u16 deserialize16(const u8 *from)
{
	return from[0] | (from[1] << 8);
}


static u32 deserialize32(const u8 *from)
{
	return from[0] | (from[1] << 8) | (from[2] << 16) | (from[3] << 24);
}


void onfi_deserializeParamPage(onfi_paramPage_t *dst, const u8 *src)
{
	hal_memcpy(dst->signature, &src[0], sizeof(dst->signature));
	...
	hal_memcpy(dst->res1, &src[15], sizeof(dst->res1));

	/* Manufacturer information block (offsets 32-79) */
	hal_memcpy(dst->devManufacturer, &src[32], sizeof(dst->devManufacturer));
	hal_memcpy(dst->devModel, &src[44], sizeof(dst->devModel));

	dst->jedecId = src[64];
	dst->dateCode[0] = src[65];
	dst->dateCode[1] = src[66];

	hal_memcpy(dst->res2, &src[67], sizeof(dst->res2));
	...
	hal_memcpy(dst->res3, &src[116], sizeof(dst->res3));
	...
	hal_memcpy(dst->vendorSpecific, &src[166], sizeof(dst->vendorSpecific));
...
const onfi_timingMode_t *onfi_getTimingModeSDR(unsigned int mode)
{
	...
	static const onfi_timingMode_t onfi_timings[] = {
		...
		{
			.tADL = 400, .tALH = 5, .tALS = 10, .tAR = 10, .tCEA = 25, .tCEH = 20, .tCH = 5, .tCHZ = 30, .tCLH = 5,
			.tCLR = 10, .tCLS = 10, .tCOH = 15, .tCR = 10, .tCR2 = 100, .tCS = 15, .tCS3 = 100, .tDH = 5, .tDS = 7,
