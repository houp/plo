...
{
	u32 sets = ((ccsidr >> 13) & 0x7fffu) + 1;
	u32 ways = ((ccsidr >> 3) & 0x3ffu) + 1;
	u32 setShift = hal_cache_getLogLine(ccsidr);
	u32 wayShift = __builtin_clz(ways - 1);
	hal_cpuDataSyncBarrier();
	for (u32 set = 0; set < sets; set++) {
		for (u32 way = 0; way < ways; way++) {
			*(cpu_common.scb + reg_offset) = (set << setShift) | (way << wayShift);
		}
	}
	hal_cpuDataSyncBarrier();
}
